...
The runner executes contract, storage, RPC, security, UI, E2E, export, license, and aggregate workspace-release gates. It fails closed on a dirty tree, zero discovered tests, command failure, missing report, candidate SHA drift, unresolved no-ship item, or an unapproved license state. Commercial spreadsheet/canvas packages may pass only in the documented backend-enforced forced-off state when entitlement is absent.
...
The enterprise default is global off: `artifact_workspace.enabled=false` and every per-kind/export flag false. After a green candidate, rollout is document first, then form/code, then flow/slides, with spreadsheet/canvas still dependent on backend license capability. `assistant_ui_runtime.enabled` and `ai_sdk_tauri_transport.enabled` are separately reversible.
