...
- `imperaos` is the canonical CLI and Python package.
- The default state root is `.imperaos`; see the [migration runbook](DESKTOP_IDENTITY_MIGRATION.md) for legacy state.
- Core installs and releases do not require desktop-control extensions.
...
Product boundary: [Agent Control Plane Product Boundary](AGENT_CONTROL_PLANE_PRODUCT_BOUNDARY.md).
Governed Memory v1 is documented in [Governed Memory Layer](GOVERNED_MEMORY_LAYER.md).

ImperaOS currently provides three control-plane surfaces:
...
> readiness evidence for broader claims.
...
- **Approval lifecycle discipline**: approvals follow `pending -> approved -> executing -> executed -> consumed`; approval alone does not authorize execution.
- **Typed contracts**: planner outputs, expert payloads, operator-panel schemas, and qualification reports are validated.
...
See [Product-Complete Closure](PRODUCT_COMPLETE_CLOSURE.md),
[First-Run Setup](FIRST_RUN_SETUP.md), and
[AI Assistant Real Runtime Guide](AI_ASSISTANT_REAL_RUNTIME_GUIDE.md).
...
> Windows support covers the core CLI, operator panel, bundled runtime, and validation workflows.
...
Computer use has been removed from the core product and Operator Panel. Its code
is preserved as the separate `imperaos-computer-use` distribution under
`extensions/computer-use`; active development is paused. Core installation,
startup, tests and release checks do not require it or platform qualification.
See [the paused extension policy](COMPUTER_USE_EXTENSION.md) for migration,
opt-in checks and conditions for reconsidering development.
...
Optional extension qualification is separate from core release readiness and cannot enable desktop control in the core product.
...
docs/COMPUTER_USE_EXTENSION.md
