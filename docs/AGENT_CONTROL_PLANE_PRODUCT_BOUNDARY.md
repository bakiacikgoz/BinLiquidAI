...
ImperaOS is a constrained, self-hosted Agent Control Plane for
...
## Paused optional extension

Computer use is retained only in the separate `imperaos-computer-use` distribution.
It is outside the core UI, CLI, installation and release requirements. Active
development is paused; see [extension policy](COMPUTER_USE_EXTENSION.md).
