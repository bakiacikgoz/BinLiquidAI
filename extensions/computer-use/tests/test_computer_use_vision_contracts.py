...
from imperaos_computer_use.models import ComputerUseMode
...
from imperaos.cli import app as core_app
...
def test_operator_capabilities_exposes_additive_vision_runtime_contract() -> None:
    result = runner.invoke(core_app, ["operator", "capabilities", "--json"])
    ...
    if current_platform().label == "windows":
        ...
        assert vision.reason_code == "COMPUTER_USE_EXTENSION_NOT_INSTALLED"
    else:
        assert vision.enabled is False
