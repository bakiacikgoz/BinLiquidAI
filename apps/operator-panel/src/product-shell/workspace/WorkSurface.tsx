export function WorkSurface({
  ...
  return <section className="ps-workspace" aria-label="Artifact workspace">
    ...
    <div><button type="button" onClick={onOpenArtifacts}>Open Artifact Workspace</button><button type="button" onClick={onOpenTerminal}>Open terminal</button><button type="button" onClick={onOpenBrowser}>Open browser</button><button type="button" onClick={onOpenPreview}>Open preview</button><button type="button" disabled title="Agent browser remains unavailable until a qualified task or deployment policy supplies a domain allowlist." data-disabled-reason="AGENT_BROWSER_CAPABILITY_UNQUALIFIED">Agent browser unavailable</button><button type="button" disabled title="A governed project-files capability is not registered in this desktop runtime." data-disabled-reason="PROJECT_FILES_CAPABILITY_UNAVAILABLE">Files unavailable</button><button type="button" disabled title="A governed data-query capability is not registered in this desktop runtime." data-disabled-reason="DATA_EXPLORER_CAPABILITY_UNAVAILABLE">Data explorer unavailable</button></div>
