...
export function WorkSurface({ taskTitle }: { taskTitle: string }) {
  ...
  return <section className="ps-workspace" aria-label="Artifact workspace">
    ...
    <div><button type="button" onClick={() => window.dispatchEvent(new CustomEvent('imperaos:open-artifact-workspace'))}>Open Artifact Workspace</button><button type="button" onClick={() => setTerminalOpen(true)}>Open terminal</button><button type="button" onClick={() => setBrowserOpen(true)}>Open browser</button></div>
