...
describe('BrowserApprovalInbox', () => {
  it('requires explicit user confirmation before opening an allowed popup', async () => {
    mocks.invoke.mockResolvedValue('imperaos-browser-approved');
    const { user } = renderOperatorPanel(<BrowserApprovalInbox />);
    await vi.waitFor(() => expect(mocks.receive).not.toBeNull());
    mocks.receive?.({ payload: { kind: 'new_window', url: 'https://docs.example.com/', mode: 'user', taskId: null } });
    ...
    expect(mocks.invoke).toHaveBeenCalledWith('browser_open', {
      request: { mode: 'user', url: 'https://docs.example.com/', taskId: undefined },
    });
  });
});
